...
Next: a *single combined* recorded demo standing up the full multi-provider topology at once with every completion path live (each piece is proven; the choreography and one Lambda firewall rule are the remaining step); **[D2]** production DNS wiring; and the deferred hardening (control-plane HA, frame v2, cold-start optimization). See TARGET_STATE.md.

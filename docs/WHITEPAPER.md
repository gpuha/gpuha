...
**Scott McDonald** · Draft v0.3 (2026-07-03) · Status: PRIVATE — do not publish before the patent/open-source decision (§8 mechanism detail is the sensitive element; see OPEN_CORE.md for the disposition table)

> Everything below describes built, tested, running code unless explicitly marked. The two-tier telemetry contract is now proven across **three independent transport legs — localhost, LAN, and public internet**. Both of those placeholders are now closed: real-GPU failover *targets* are proven in Phase C'-a (intra-pool) and Phase C'-c (cross-cloud), and the multi-cloud orchestrator (`gpuha up/down/reap`) is built and proven live in Phase O. Everything through Phase O is done.
...
Running today: a permanent always-on control plane (Linode 2GB, ~$12/mo) hosting Tier-1 (`tier1_dns.py`) and the whale (`whale.py`) as boot-enabled systemd services. Canonical code is durable in a private git repo. The two-tier telemetry contract is proven across localhost, LAN, and public-internet transport. Total project spend to date: a few dollars of GPU time plus the plane's monthly rate; five legacy boxes (~$68/mo) were recovered and deleted, so cleanup roughly funds the plane.

**[PHASE O]** (now built and proven live) -- the multi-cloud orchestrator (`gpuha up/down` — declarative topology, best-effort capacity acquisition across GCP/AWS/RunPod/Lambda, teardown to zero idle cost); **[PHASE C′]** the full recorded demo with real GPU pools on both ends of the failover; **[D2]** production DNS wiring. See TARGET_STATE.md.

**[PHASE E: add demo recording links, /__stats snapshots, dig transcripts; final edit pass for tone and length.]**
